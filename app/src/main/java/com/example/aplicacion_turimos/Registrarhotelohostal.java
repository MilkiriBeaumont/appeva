...
import com.example.aplicacion_turimos.modelos.hotel;
import com.google.firebase.firestore.CollectionReference;
import com.google.firebase.firestore.FirebaseFirestore;

public class Registrarhotelohostal extends AppCompatActivity {
    FirebaseFirestore db;  // Instancia de Firestore
    CollectionReference eHotel; // Referencia a la colección "usuario"
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        eHotel = db.collection("hotel");

    }

    public void registrar(View v) {
        //Recuperando al view
        EditText eHotel = (EditText) findViewById(R.id.eHotel);
        //Recuperando el valor del  view
        String hotel = eHotel.getText().toString();
        //Recuperando al view
        ...
        hotel nuevohotel = new hotel(hotel, correo, password,password2);

        // Agregar el nuevo usuario a Firestore
        agregarUsuarioFirestore(nuevohotel);
    }

    private void agregarUsuarioFirestore(hotel hotel) {
        // Obtener una referencia a la colección "usuario" (debes crearla previamente en Firestore)
        db.collection("hotel")
                .add(hotel) // Utiliza .add() para que Firestore genere un ID automáticamente
                .addOnSuccessListener(documentReference -> {
                    ...
                    db.collection("usuario")
                            ...
                            .addOnSuccessListener(aVoid -> {
                                // Registro exitoso
                                Toast.makeText(this, "Registro exitoso con ID: " + hotelId, Toast.LENGTH_SHORT).show();

                                // Guardar el ID del usuario en las preferencias compartidas
                                SharedPreferences datos = PreferenceManager.getDefaultSharedPreferences(this);
                                SharedPreferences.Editor editor = datos.edit();
                                editor.putString("_Id", hotelId); // Guardar el ID del hotel
