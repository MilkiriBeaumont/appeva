...
import com.example.aplicacion_turimos.modelos.lugar;
import com.google.firebase.firestore.CollectionReference;
import com.google.firebase.firestore.FirebaseFirestore;

public class Registrarlugar extends AppCompatActivity {
    FirebaseFirestore db;  // Instancia de Firestore
    CollectionReference eLugar; // Referencia a la colección "usuario"


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_registrarlugar);
        // Inicializar la instancia de Firestore
        db = FirebaseFirestore.getInstance();

        // Obtener una referencia a la colección "usuario"
        eLugar = db.collection("lugar");

    }

    public void registrar(View v) {
        //Recuperando al view
        EditText eLugar = (EditText) findViewById(R.id.eLugar);
        //Recuperando el valor del  view
        String lugar = eLugar.getText().toString();
        //Recuperando al view
        ...
        com.example.aplicacion_turimos.modelos.lugar nuevolugar = new lugar(lugar, correo, password,password2);

        // Agregar el nuevo usuario a Firestore
        agregarUsuarioFirestore(nuevolugar);
    }

    private void agregarUsuarioFirestore(lugar lugar) {
        // Obtener una referencia a la colección "usuario" (debes crearla previamente en Firestore)
        db.collection("lugar")
                .add(lugar) // Utiliza .add() para que Firestore genere un ID automáticamente
                .addOnSuccessListener(documentReference -> {
                    // Registro exitoso, obtén el ID del lugar
                    String lugarId = documentReference.getId();
                    // Asignar el ID generado automáticamente al atributo _id del usuario
                    lugar.set_Id(lugarId);
                    ...
                    db.collection("usuario")
                            ...
                            .addOnSuccessListener(aVoid -> {
                                // Registro exitoso
                                Toast.makeText(this, "Registro exitoso con ID: " + lugarId, Toast.LENGTH_SHORT).show();

                                // Guardar el ID del usuario en las preferencias compartidas
                                SharedPreferences datos = PreferenceManager.getDefaultSharedPreferences(this);
                                SharedPreferences.Editor editor = datos.edit();
                                editor.putString("_Id", lugarId); // Guardar el ID del hotel
